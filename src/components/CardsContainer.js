  ...
  return (
    ...
    >
      <div
        ...
        className="relative z-20 flex h-16 shrink-0 justify-center border-b backdrop-blur-lg md:h-[70px] border-borderMain/50 ring-borderMain/50 divide-borderMain/50 dark:divide-borderMainDark/50  dark:ring-borderMainDark/50 dark:border-borderMainDark/50 bg-transparent"
      >
        ...
        <div className="ease relative flex w-full items-center justify-between px-md duration-150 md:px-lg max-w-screen-md">
          <h1 className="text-white flex items-center gap-1 ml-3 default font-display text-2xl md:text-3xl text-textMain dark:text-textMainDark selection:bg-super/50 selection:text-textMain dark:selection:bg-superDuper/10 dark:selection:text-superDark">
            <svg
              ...
              data-prefix="fak"
              data-icon="discover"
              className="svg-inline--fa fa-discover fa-sm "
              role="img"
        ...
        <div className="mr-md flex items-center md:hidden">
          <button
            type="button"
      ...
      <div className="mx-auto h-full w-full max-w-screen-md px-md md:px-lg">
            ...
            <div className="flex grid-cols-3 flex-col gap-5 mt-1 md:grid px-5 ">
              {articleList.map((item, index) => {
                ...
                if (index % 4 === 0) {
                  return (
                    <BigCard
                      key={item.link} // Use a unique key for each card
                      title={item.title}
                      description={item.description}
                      imageUrl={item.imageUrl}
                      author={item.author}
                      authorAvatar={item.authorImage}
                      href={item.link}
                    />
                  );
                }

                // Render SmallCard for indices 1, 2, 3 (1, 2, 3, 5, 6, 7, ...)
                return (
                  <SmallCard
                    key={item.link} // Use a unique key for each card
                    title={item.title}
